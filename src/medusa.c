...
#define VERSION_SVN "$Id: medusa.c 1666 2012-02-09 19:40:02Z jmk $" 
...
int startServerThreadPool(sAudit *_psAudit)
{
  ...
  for (iServerId = 0; iServerId < _psAudit->iHostCnt; iServerId++)
  {
    ...
    if (nAddHost)
    {
      ...
      //if ( thr_pool_queue(server_pool, startLoginThreadPool, (void *)psServer + iServerId * sizeof(sServer)) < 0 )
