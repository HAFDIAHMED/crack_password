...
#define MODULE_VERSION_SVN "$Id: ssh.c 1681 2012-02-29 19:12:47Z jmk $"
...
pthread_mutex_t ptmLibSSH2Mutex;
...
int tryLogin(_SSH2_DATA* _psSessionData, int hSocket, LIBSSH2_SESSION *session, sLogin** login, char* szLogin, char* szPassword);
...
int initModule(sLogin* psLogin, _SSH2_DATA *_psSessionData)
{
  ...
  pthread_mutex_lock(&ptmLibSSH2Mutex);
  
  // Utilize libssh2_init() once more distro include libssh2 1.2.5 or greater
  //if (libssh2_init(0))
  //{
  ...
  pthread_mutex_unlock(&ptmLibSSH2Mutex);
  ...
  {  
    switch (nState)
    {
      ...
      case MSTATE_RUNNING:
        ...
        ssh2_session_data.iAnswerCount = 0;
        nState = tryLogin(_psSessionData, hSocket, session, &psLogin, psCredSet->psUser->pUser, psCredSet->pPass);
...
int tryLogin(_SSH2_DATA* _psSessionData, int hSocket, LIBSSH2_SESSION *session, sLogin** psLogin, char* szLogin, char* szPassword)
{
  char *pErrorMsg = NULL;
