...
#define MODULE_VERSION_SVN "$Id: smbnt.c 1682 2012-02-29 19:13:03Z jmk $"
...
int SMBNegProt(int hSocket, _SMBNT_DATA* _psSessionData)
{
  ...
  if (medusaSend(hSocket, (char *) buf, iLength, 0) < 0)
  {
    writeError(ERR_ERROR, "%s failed: medusaSend was not successful", MODULE_NAME);
  }
...
unsigned long SMBSessionSetup(int hSocket, _SMBNT_DATA *_psSessionData, char* szLogin, char* szPassword)
{
  ...
  else if (_psSessionData->security_mode == PLAINTEXT)
  {
    ...
    */
    strncpy(buf + iOffset, szPassword, 256);
  }
  else
  {
  ...
  sprintf(buf + iOffset + iByteCount, "Unix"); 
  iByteCount += strlen("Unix") + 1; /* NULL pad OS name */
  sprintf(buf + iOffset + iByteCount, "Samba"); 
  ...
  if (medusaSend(hSocket, (char *) buf, iOffset + iByteCount, 0) < 0)
  {
    writeError(ERR_ERROR, "%s failed: medusaSend was not successful", MODULE_NAME);
  }
