...
#define MODULE_VERSION_SVN "$Id: vnc.c 1725 2012-05-23 17:00:33Z jmk $"
...
int vncSessionSetup(int hSocket, _VNC_DATA* _psSessionData)
{
  ...
  */
  if ((strncmp(bufReceive + 20, "Too many security failures", 26) == 0) || (strncmp(bufReceive + 20, "Too many authentication failures", 32) == 0))
  {
  ...
  else if (iServerProtocolVersion == 3)
  {
    ...
    switch (bufReceive[3])
    {
      case 0x00:  /* connection failure */
        ...
        if ((nReceiveBufferSize == 42) && (strncmp(bufReceive + 8, "Your connection has been rejected.", 34) == 0)) 
      ...
      case 0xfffffffa: /* UltaVNC MS-Logon */
  ...
  */
  else if ((iServerProtocolVersion == 7) || (iServerProtocolVersion == 8))
  {
    ...
    if (bufReceive[0] == 0)
    {
      ...
      /* Server is probably in anti-brute force mode (UltraVNC) */
      if (strncmp(bufReceive + 5, "Your connection has been rejected.", 34) == 0)
        return SESSION_MAX_AUTH_ULTRAVNC;
      else
        return SESSION_FAILURE;
    }
      ...
      for (i = 0; i <= nSecurityTypes; i++)
      {
        ...
        switch (szSecurityTypes[i])
        {
          ...
          case 0x82:  /* 130: External Authentication */
            writeError(ERR_ERROR, "VNC Session Setup - RealVNC (Type %d). RealVNC native authentication mode is NOT currently supported.", szSecurityTypes[i]);
            break;
...
int sendAuthMSLogin(int hSocket, _VNC_DATA* _psSessionData, char* szLogin, char* szPassword)
{
  ...
  { 
    strncpy(ms_user, _psSessionData->szDomain, strlen(_psSessionData->szDomain));
    strncat(ms_user, "\\", 1);
    strncat(ms_user, szLogin, strlen(szLogin));
  }
  else
    strncpy(ms_user, szLogin, 256);

  strncpy(ms_passwd, szPassword, 64);
...
{
  char szExit[] = { 0x00, 0x00, 0x00, 0x00, 0x05, 0x1D, 0x03, 0x20 }; 
  ...
  else if (nReceiveBufferSize >= 4)
  {
    switch (bufReceive[3])
    {
      case 0x00:
        ...
        break;
      case 0x01:
        if ((nReceiveBufferSize > 8) && (strstr(bufReceive + 8, "Connection rejected by user") != NULL))
        {
