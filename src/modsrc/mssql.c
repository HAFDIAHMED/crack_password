...
#define MODULE_VERSION_SVN "$Id: mssql.c 1719 2012-05-23 16:56:04Z jmk $"
...
int connectMSSQL(sLogin *_psLogin, _MODULE_DATA *_psSessionData)
{
  ...
  */
  ...
  { 
    ...
    if (bufReceive == NULL)
    {
      writeError(ERR_ERROR, "[%s] SQL server (%s) did not response to port query request. Using default value of 1433/tcp.", MODULE_NAME, _psLogin->psServer->pHostIP);
      nPort = 1433;
    } 
    else if (bufReceive[0] == 0x05)
    {
      ...
      nSQLInstance = 1;
      szTmp = bufReceive + 3;
      while (szTmp = strstr(szTmp, "ServerName;"))
      {
        szTmp1 = strstr(szTmp, ";;");
